...
Rcpp::List RcppUtilities::create_prediction_object(const std::vector<Prediction>& predictions) {
    ...
    result.push_back(RcppUtilities::create_tree_prediction_matrix(predictions), "tree.predictions");
    return result;
};
...
Rcpp::NumericMatrix RcppUtilities::create_tree_prediction_matrix(const std::vector<Prediction>& predictions) {
    if (predictions.empty()) {
        return Rcpp::NumericMatrix(0);
    }
    
    size_t num_trees = predictions.at(0).forest_size();
    Rcpp::NumericMatrix result(predictions.size(), num_trees);
    
    for (size_t i = 0; i < predictions.size(); i++) {
        const std::vector<double>& prediction = predictions[i].get_tree_predictions();
        for (size_t j = 0; j < prediction.size(); j++) {
            double value = prediction[j];
            result(i, j) = value;
        }
